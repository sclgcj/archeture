...
)
{
	...
	case cJSON_String:
		if (!strcmp(input->valuestring, req->valuestring)) 
			...
			GINFO("%s value is not the same, req = %s, input = %s\n", 
	...
	case cJSON_True:
		if (input->type == cJSON_String) {
			if (req->valueint == atoi(input->valuestring) ||
				...
				GINFO("%s value is not the same, req = %s,%lf, input = %s\n", 
			...
			if (req->valueint == input->valueint || 
				...
				GINFO("%s value is not the same, req = %s,%f, input = %d\n", 
					input->string, req->valuestring, 
